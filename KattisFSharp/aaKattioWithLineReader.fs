module KattisFSharp.aaKattioWithLineReader
...
type Tokenizer(inStream : Stream) =
    ...
    member this.NextLine() =
        if pos < Array.length tokens then
            // Continue processing the remaining tokens on the current line
            let remainingTokens = tokens.[pos..]
            pos <- Array.length tokens  // Move position to end to avoid reprocessing
            remainingTokens
        else
            // Read the next line if we're at the end of the current tokens
            let line = reader.ReadLine()
            if line = null then
                [||]
            else
                tokens <- line.Split(' ')
                pos <- Array.length tokens
                tokens
...
type Scanner(inStream : Stream) =
    ...
    member this.NextLineTokens() =
        this.NextLine() |> List.ofArray
